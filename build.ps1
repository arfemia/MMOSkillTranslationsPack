...
# --- PER-PACK ---
$ZipName           = 'MMOSkillTranslationsPack.zip'
# manifest.json is excluded on purpose: the repo carries it for git history, but the
# deploy pipeline supplies it separately at install time.
$ExtraExcludeNames = @('manifest.json')
...
try {
    $files = Get-ChildItem -Path $pack -Recurse -File -Force | Where-Object {
        ...
        $top = ($rel -split '/')[0]
        ($_.Name -notin $excludeNames) -and ($top -notin $excludeDirs)
    }
